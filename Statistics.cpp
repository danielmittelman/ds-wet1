...
StatusType Statistics::UpdateDownloads(int groupBase, int multiplyFactor) {
	...
	try {
		...
		doUpdateDownloadsInTree(&(mAppsByDownloadCount), groupBase, multiplyFactor, false);

		// Update the k OSVersion trees, this time not updating the actual data
		OSVersionsList::Iterator it = mOSVersionsList.getFront();
		for(int i = 0 ; i < mOSVersionsList.getSize(); i++) {
			AppsByDownloadCountTree* tree = mOSVersionsList.getAppsByDownloadCountTree(it->versionCode);
			doUpdateDownloadsInTree(tree, groupBase, multiplyFactor, true);
			it++;
		}
